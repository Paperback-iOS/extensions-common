...
export class APIWrapper {
  ...
  async filterUpdatedManga(
    ...
    while (hasResults && data.data) {
      let results: MangaUpdates | null = source.filterUpdatedManga(
        data.data,
        request.metadata
      );
  ...
  async getHomePageSections(source: Source) {
    ...
    let sections: any = [];
    for (let key of keys) {
      for (let section of request[key].sections) sections.push(section);
      configs.push(request[key].request);
    }

    try {
      var data: any = await Promise.all(configs.map((axios as any).request));
